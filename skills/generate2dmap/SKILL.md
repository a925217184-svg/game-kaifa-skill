...
This skill is image-generation-first for visual assets. **Image generation is performed by the Lovart backend (preferred) or 即梦 Dreamina CLI — never by Codex/Grok built-in `image_gen`.** The agent must use one of these real backends for base maps, in-world reference mockups, dressed references, stage references, prop sheets, prop sprites, tileset art, parallax layers, battle backgrounds, and other visible map assets.

### Backend Selection & Cost Notification (MANDATORY)

**Before EVERY image generation call, the agent MUST tell the user:**

1. **Which backend** will be used (`Lovart` or `Dreamina CLI`)
2. **Which mode** (`fast` = consumes Lovart credits / `unlimited` = free but may queue)
3. **What is being generated** (one-line description, e.g. "2K 16:9 side-scroll sky layer")
4. **Estimated cost** (Lovart fast mode = ~N credits per generation; Dreamina = consumes account credits)

Example notification:
> 📸 **即将生图** | 后端: **Lovart** (mode: **fast**, 扣信用点) | 内容: 1536×864 远景 parallax 层 × 2 段 | 预计消耗: ~2 次生成

The user may reply to switch backend or mode. **Do NOT generate until the user acknowledges or does not object within one turn.**

### Available Backends

| Backend | Command / API | Mode | Cost |
|---|---|---|---|
| **Lovart** (preferred) | Python `LovartBackend().generate(prompt, ...)` from `UI拆图Skill/scripts/backends/lovart.py` | `fast` (default) = credits, no queue | `unlimited` = free, may queue |
| **Dreamina CLI** | `dreamina text2image --prompt="..." --resolution_type=2k` | Consumes Dreamina account credits | Free tier has daily limits |

### Lovart Usage

```python
from sys.path import os.path.join(os.path.expanduser("~"), ".workbuddy/skills/UI拆图Skill/scripts")
sys.path.insert(0, SCRIPTS_DIR)
from backends.lovart import LovartBackend

backend = LovartBackend()  # reads AK/SK from ~/.workbuddy/config/lovart.json
path = backend.generate(
    prompt="your prompt here",
    ratio="16:9",        # or "1:1", "4:3", "3:4", "9:16", etc.
    resolution="2k",      # or "1k"
    out_path="output.png",
    poll=300,
)
...
Set `LOVART_MODE=unlimited` in the environment to avoid burning credits during large batch operations.

### Dreamina CLI Usage

```bash
# Text-to-image
dreamina text2image --prompt="your prompt" --resolution_type=2k --output ./output.png

# Image-to-image (with reference)
dreamina image2image --input reference.png --prompt="your prompt" --output ./output.png
```

Run `dreamina login` once to authenticate (OAuth device flow). Run `dreamina user_credit` to check remaining credits.
...
2. Immediately before the next image generation call (Lovart or Dreamina), make that exact image visible in conversation context. If it is a local file, present it as an image attachment.
...
- `visual_asset_source`: lovart | dreamina | existing_assets | procedural_placeholder
...
- Use **Lovart** (preferred) or **Dreamina CLI** as the visual asset source. See "Backend Selection & Cost Notification" above.
...
2. Choose the pipeline axes.
   ...
   - Select `visual_asset_source`. Default to **Lovart**; use `dreamina` for Dreamina CLI; use `existing_assets` only when the project already has suitable art; use `procedural_placeholder` only when explicitly requested.
   - Treat `hybrid` as a result of combining axes, not as a primary category.

3. Produce assets.
   - **[COST NOTIFY]** Before each image generation, tell the user: backend (Lovart/Dreamina), mode (fast/unlimited), what you're generating, estimated cost. Wait for acknowledgment.
   - Write the creative prompts manually and use **Lovart backend** or **Dreamina CLI** for visible map art unless the user explicitly chose existing assets or procedural placeholders.
   - For baked raster maps, generate one background with Lovart/Dreamina, or edit/use an existing image when supplied, then add optional collision/zones metadata.
...
2. Make the base image visible in conversation context. If the base is a local file, present it as an image attachment immediately before the next Lovart/Dreamina generation; do not rely on a path string as the reference.
...
4. For each visible runtime object, choose exactly one asset strategy:
   - generate a separate transparent asset with `$generate2dsprite` or direct Lovart/Dreamina generation
...
2. Make the background visible in conversation context. If it is a local file, present it as an image attachment immediately before the next Lovart/Dreamina generation; do not rely on a path string as the reference.
