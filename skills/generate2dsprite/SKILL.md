...
description: "Generate and postprocess general 2D game assets and animation sheets: pixel-art sprites, clean HD map props, creatures, characters, NPCs, spells, projectiles, impacts, props, summons, and transparent GIF exports. Image generation uses Lovart backend (HMAC-SHA256 AK/SK) or 即梦 Dreamina CLI — never Codex/Grok built-in image_gen. Use when the agent should infer the asset plan from a natural-language request, match a reference or map art style, call Lovart/Dreamina for solid-magenta raw sheets, and use the local processor only for chroma-key cleanup, frame extraction, alignment, QC, and transparent exports."
---
...
## Image Generation Backend (MANDATORY)

**Sprite art is generated by Lovart (preferred) or Dreamina CLI — NEVER by Codex/Grok built-in `image_gen`.**

### Cost Notification Rule

**Before EVERY image generation call, the agent MUST tell the user:**

1. **Which backend**: `Lovart` or `Dreamina CLI`
2. **Which mode**: `fast` (Lovart, consumes credits) / `unlimited` (Lovart, free/queued) / `Dreamina` (consumes account credits)
3. **What**: one-line description (e.g. "2×2 idle grid, 512×512 per cell, pixel art style")
4. **Estimated cost**: ~N credits or Dreamina credit count

Example:
> 📸 **即将生图** | 后端: **Lovart** (mode: **fast**, 扣信用点) | 内容: 2×2 idle 网格 512×512/cell 像素风 | 预计: ~1 次生成

**Do NOT generate until the user acknowledges or does not object.**

### Available Backends

| Backend | How to call | Mode | Cost |
|---|---|---|---|
| **Lovart** (preferred) | `LovartBackend().generate(prompt, ratio, resolution, out_path)` | `fast` = credits / `unlimited` = free+queue | See lovart.py |
| **Dreamina CLI** | `dreamina text2image --prompt="..." --resolution_type=2k` | Consumes Dreamina credits | Daily free tier limits |

### Lovart Quick Reference

```python
import sys, os
SCRIPTS = os.path.join(os.path.expanduser("~"), ".workbuddy/skills/UI拆图Skill/scripts")
sys.path.insert(0, os.path.join(SCRIPTS, "backends"))
from lovart import LovartBackend

be = LovartBackend()  # reads AK/SK from ~/.workbuddy/config/lovart.json
raw = be.generate(
    prompt="top-down 2D pixel art RPG overworld character, 2x2 grid...",
    ratio="1:1", resolution="2k",
    out_path="raw-sheet.png", poll=300,
)
```

Set `LOVART_MODE=unlimited` to save credits during batch work.

### Dreamina CLI Quick Reference

```bash
# Login once
dreamina login   # OAuth device flow — open URL + enter code

# Generate
dreamina text2image \
  --prompt "top-down 2D pixel art character, 2x2 grid, solid #FF00FF background..." \
  --resolution_type=2k \
...
dreamina image2image \
  --input reference.png \
  --prompt "same character, attack pose, 2x3 grid..." \
  --output ./attack-sheet.png
...
- Use **Lovart backend** or **Dreamina CLI** for every raw image. See "Image Generation Backend" above.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from Lovart or Dreamina CLI.
- When the user provides or implies a visual reference, use Lovart/Dreamina image-to-image (with the reference attached) only after the reference image is visible in the conversation context. If the reference is a local file, present it as an image attachment first; do not rely on a filesystem path in the prompt as the visual reference.
...
- For high-value grounded player/hero body actions, prefer a character anchor sheet when consistent scale or feet placement matters: repeat one accepted master frame at the intended size and feet line in every cell, then use that sheet as a scale/root template while asking Lovart/Dreamina to change only the poses. Do not use a grounded anchor sheet for jumps, knockback, airborne motion, projectiles, or FX.
...
- Do not ask Lovart/Dreamina to generate unrelated action rows in one raw sheet, such as `row 1 idle, row 2 run, row 3 shoot, row 4 jump`, for a controllable hero or main character.
- Do not ask Lovart/Dreamina to generate raw single-row action strips such as `1x4 idle`, `1x4 run`, `1x4 shoot`, or `1x4 jump` for a controllable hero, character, creature, NPC, enemy, summon, or animated prop.
...
If a layout guide is useful, generate one before calling Lovart/Dreamina:
...
Then make the guide visible in the conversation context and tell Lovart/Dreamina to use it only for invisible slot count, spacing, centering, and safe padding. The output must not reproduce guide boxes, safe-area rectangles, center marks, labels, borders, guide background.
...
Make both the master frame and anchor sheet visible to Lovart/Dreamina. State that the master locks identity/style and the anchor sheet locks slot positions, camera distance, standing-equivalent scale, body root, feet line, and padding. Ask the model to change only the action poses and never reproduce guides, borders, labels, or separators.

### 3. Generate the raw image

Use **Lovart backend** (preferred) or **Dreamina CLI**. See "Image Generation Backend" above for backend selection, cost notification, and usage examples.

**[COST NOTIFY]** Before generating, tell the user: backend, mode, what you're generating, estimated cost. Wait for acknowledgment.
...
- the raw PNG will be at the `out_path` you specified (Lovart) or `--output` path (Dreamina)
