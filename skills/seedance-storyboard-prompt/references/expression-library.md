...
> 引用方式：`@角色 <站位动作>，表情【极致震惊】——双眼…；嘴角…；面部…`；可追加过渡修饰（如「表情【微愠不悦】→ 转为【暴怒发火】」），过渡修饰同样要双写展开。
> **双写规则（2026-09-20 升级为全流程双写，不得再有例外）**：
> - **【全流程双写】任何交付物（分镜脚本、Seedance 提示词）里，引用词条时一律写「词条名 + 展开描述」**：`表情【极致震惊】——双眼骤然睁大瞪圆、瞳孔收缩、眼白增多，眉头高挑上扬；嘴唇微张、嘴角向两侧平拉发僵、下颌微垂；额肌收紧抬头纹显现、面部肌肉僵住、血色略褪`。展开保留「眼部 + 嘴角 + 面部」三个维度。**禁止在任何地方只写【词条名】裸奔**——模型不认词条名，人也不该再手动展开。
...
| 2026-09-20 | 删除与本库顶部「全流程双写」自相矛盾的旧条款（原第 6 行「分镜脚本阶段写简称即可」）——该条款会导致脚本直接被拿去用时模型读不懂，与用户二次纠正冲突；引用方式示例同步改为双写展开式 |
